...
if(!class_exists('Redux_Options') ){
    ...
    if(!defined('Redux_OPTIONS_DIR')) {
        define('Redux_OPTIONS_DIR', get_template_directory() . '/options/options/');
    }

    if(!defined('Redux_OPTIONS_URL')) {
        define('Redux_OPTIONS_URL', get_template_directory_uri() . '/options/options/');
    }
    ...
    class Redux_Options {
        ...
        */
        function __construct($sections = array(), $args = array(), $extra_tabs = array()) {
            ...
            $defaults['menu_title'] = __('Options', 'dfd');
            $defaults['page_icon'] = 'awesome-themes';
            $defaults['page_title'] = __('Options', 'dfd');
            ...
            $defaults['footer_credit'] = sprintf(__('<span id="footer-thankyou">Options panel created using <a href="%s" target="_blank">Redux Framework</a> v%s</span>', 'dfd'), $this->framework_url, $this->framework_version);
            $defaults['help_tabs'] = array();
            $defaults['help_sidebar'] = __('', 'dfd');
        ...
        function generate_options_css($newdata) {
            ...
            /** Capture CSS output **/
            ob_start();
            require locate_template('/options/options/styles.php');
            $css = ob_get_clean();
        ...
        */
        function _enqueue() {
            ...
            wp_localize_script('redux-opts-js', 'redux_opts', array('reset_confirm' => __('Are you sure? You will lose all custom values.', 'dfd'), 'opt_name' => $this->args['opt_name']));
        ...
        */
        function _register_setting() {
			if (
					isset($_GET['upgrade_theme']) &&
					1 == $_GET['upgrade_theme'] &&
					isset($_GET['_nonce']) &&
					wp_verify_nonce($_GET['_nonce'], 'sb_theme_upgrade')
			) {
        ...
        */
        function _options_page_html() {
            ...
            echo ' &nbsp; ';
            submit_button(__('Reset to Defaults', 'dfd'), 'secondary', $this->args['opt_name'] . '[defaults]', false);
            ...
            if(isset($_GET['settings-updated']) && $_GET['settings-updated'] == 'true' && get_transient('redux-opts-saved') == '1') {
                if(isset($this->options['imported']) && $this->options['imported'] == 1) {
                    echo '<div id="redux-opts-imported">' . apply_filters('redux-opts-imported-text-' . $this->args['opt_name'], __('<strong>Settings Imported!</strong>', 'dfd')) . '</div>';
                } else {
                    echo '<div id="redux-opts-save">' . apply_filters('redux-opts-saved-text-' . $this->args['opt_name'], __('<strong>Settings Saved!</strong>', 'dfd')).'</div>';
                }
                delete_transient('redux-opts-saved');
            }

            echo '<div id="redux-opts-save-warn">' . apply_filters('redux-opts-changed-text-' . $this->args['opt_name'], __('<strong>Settings have changed, you should save them!</strong>', 'dfd')) . '</div>';
            echo '<div id="redux-opts-field-errors">' . __('<strong><span></span> error(s) were found!</strong>', 'dfd') . '</div>';
            echo '<div id="redux-opts-field-warnings">' . __('<strong><span></span> warning(s) were found!</strong>', 'dfd').'</div>';
            ...
            if(true === $this->args['show_import_export']) {
                ...
                echo '<a href="javascript:void(0);" id="import_export_default_section_group_li_a" class="redux-opts-group-tab-link-a" data-rel="import_export_default">' . $icon . ' <span>' . __('Import / Export', 'dfd') . '</span></a>';
                echo '</li>';
            ...
            if(true === $this->args['dev_mode']) {
                ...
                echo '<a href="javascript:void(0);" id="dev_mode_default_section_group_li_a" class="redux-opts-group-tab-link-a custom-tab" data-rel="dev_mode_default">' . $icon . ' <span>' . __('Dev Mode Info', 'dfd') . '</span></a>';
                echo '</li>';
            }
            ...
            if(true === $this->args['show_import_export']) {
                ...
                echo '<h3>' . __('Import / Export Options', 'dfd') . '</h3>';
                echo '<h4>' . __('Import Options', 'dfd') . '</h4>';
                echo '<p><a href="javascript:void(0);" id="redux-opts-import-code-button" class="button-secondary">' . __('Import from file', 'dfd') . '</a> <a href="javascript:void(0);" id="redux-opts-import-link-button" class="button-secondary">' . __('Import from URL', 'dfd') . '</a></p>';

                echo '<div id="redux-opts-import-code-wrapper">';

                echo '<div class="redux-opts-section-desc">';
                echo '<p class="description" id="import-code-description">' . apply_filters('redux-opts-import-file-description', __('Input your backup file below and hit Import to restore your sites options from a backup.', 'dfd')) . '</p>';
                echo '</div>';
                ...
                echo '<div class="redux-opts-section-desc">';
                echo '<p class="description" id="import-link-description">' . apply_filters('redux-opts-import-link-description', __('Input the URL to another sites options set and hit Import to load the options from that site.', 'dfd')) . '</p>';
                echo '</div>';
                ...
                echo '<p id="redux-opts-import-action"><input type="submit" id="redux-opts-import" name="' . $this->args['opt_name'] . '[import]" class="button-primary" value="' . __('Import', 'dfd') . '"> <span>' . apply_filters('redux-opts-import-warning', __('WARNING! This will overwrite any existing options, please proceed with caution!', 'dfd')) . '</span></p>';
                echo '<div id="import_divide"></div>';

                echo '<h4>' . __('Export Options', 'dfd') . '</h4>';
                echo '<div class="redux-opts-section-desc">';
                echo '<p class="description">' . apply_filters('redux-opts-backup-description', __('Here you can copy/download your current option settings. Keep this safe as you can use it as a backup should anything go wrong, or you can use it to restore your settings on this site (or any other site).', 'dfd')).'</p>';
                echo '</div>';

                echo '<p><a href="javascript:void(0);" id="redux-opts-export-code-copy" class="button-secondary">' . __('Copy', 'dfd') . '</a> <a href="'.add_query_arg(array('feed' => 'reduxopts-'.$this->args['opt_name'], 'action' => 'download_options', 'secret' => md5(AUTH_KEY.SECURE_AUTH_KEY)), site_url()).'" id="redux-opts-export-code-dl" class="button-primary">Download</a> <a href="javascript:void(0);" id="redux-opts-export-link" class="button-secondary">' . __('Copy Link', 'dfd') . '</a></p>';
            ...
            if(true === $this->args['dev_mode']) {
                ...
                echo '<h3>' . __('Dev Mode Info', 'dfd') . '</h3>';
                echo '<div class="redux-opts-section-desc">';
