...
class MVB_Presentation_Boxes_Left_Icon
{
    /**
    ...
    {
        return array(
            'title'           =>      __('Presentation Boxes Left icon', 'dfd'),
            'description'     =>      __('Add a presentation box with left icon', 'dfd'),
    ...
    public static function render( $atts, $content = null )
    {
		...
		foreach ($load['r_items'] as $k=>$row) {
			# Alignment
			$load['r_items'][$k]['alignment'] = 1; // Align top
		}
		
		$load['few_rows'] = 1;
		$load['module_class'] = 'features_module_left_icon features_module-eq-height-simple';
		$load['box_hover_with_class'] = '';
