...
class crum_login_widget_best_sellers extends WP_Widget {
	...
	function widget( $args, $instance ) {
		...
		if ( $r->have_posts() ) {
			...
			while ( $r->have_posts()) {
				...
				$thumbnail_url = has_post_thumbnail() ? get_the_post_thumbnail( $r->post->ID, array(120, 120) ) : woocommerce_placeholder_img( 'shop_thumbnail' );

				if (function_exists('aq_resize')) {
					$thumbnail_url = has_post_thumbnail() ? wp_get_attachment_url( get_post_thumbnail_id($r->post->ID) ) : woocommerce_placeholder_img_src();
					$thumbnail_url = aq_resize($thumbnail_url, 80, 80, true);
					
					$thumbnail_url = '<img src="'.$thumbnail_url.'" alt="" />';
				}
				
				echo '<li>
					<div class="product_thumbnail">' . $thumbnail_url .'</div>
	...
	function form( $instance ) {
		...
		?>
		<p><label for="<?php echo $this->get_field_id('title'); ?>"><?php _e( 'Title:', 'dfd' ); ?></label>
		...
		<label for="<?php echo $this->get_field_id('hide_free'); ?>"><?php _e( 'Hide free products', 'dfd' ); ?></label></p>
