...
*/
	...
	function widget($args, $instance) {
		...
		$login_form_args = array(
			...
			'label_lost_password' => __('I lost my password', 'dfd'),
		);
	...
	$defaults = array(
		...
		'placeholder_username' => __( 'Enter your login', 'dfd' ),
		'label_password' => __( 'Password', 'dfd' ),
		'placeholder_password' => __( 'Enter your password', 'dfd' ),
	...
	$form = '
		<form name="' . $args['form_id'] . '" id="' . $args['form_id'] . '" action="' . esc_url( site_url( 'wp-login.php', 'login_post' ) ) . '" method="post">
			...
			. ( $args['lost_password'] ? '<p class="login-lost-password"><label>'
					. '<a href="' . wp_lostpassword_url() . '">' . esc_attr( $args['label_lost_password'] ) . '</a></label></p>' : '' )
	...
	function logout_form() {
		echo '<p class="login-logout"><a class="button" href="'.wp_logout_url().'"><i class="icon-log-out-1"></i>'.__('Logout', 'dfd').'</a></p>';
	}
