...
if ( class_exists( 'Theme_Upgrader' ) && ! class_exists( 'Envato_WordPress_Theme_Upgrader' ) ) {
    ...
    class Envato_WordPress_Theme_Upgrader extends Theme_Upgrader 
    {
        ...
        protected function do_upgrade_theme( $installed_theme_name, $marketplace_theme_id ) 
        {
			...
			$args = array(
				'package' => $this->api->wp_download( $marketplace_theme_id )
			);
			$result = $this->upgrade( $installed_theme_name, $args );
        ...
        public function upgrade_strings() {
            ...
            $this->strings['downloading_package'] = __( 'Downloading upgrade package from the Envato API&#8230;', 'dfd' );
        }
        ...
        public function install_strings() {
            ...
            $this->strings['downloading_package'] = __( 'Downloading install package from the Envato API&#8230;', 'dfd' );
        }
...
if ( class_exists('Envato_WordPress_Theme_Upgrader') && !class_exists('DFD_Envato_WordPress_Theme_Upgrader') ) {
	class DFD_Envato_WordPress_Theme_Upgrader {
		private $_upgrader;
		
		public function __construct() {
		}
		
		private function _get_upgrader() {
			if (!$this->_upgrader) {
				$this->_upgrader = new Envato_WordPress_Theme_Upgrader(
					DfdThemeSettings::get('themeforest_username'), 
					DfdThemeSettings::get('themeforest_api_key')
				);
			}
		...
		public function continueChecking() {
			...
			$lastCheck = get_option( 'envato-toolkit-last-check' );
			if ($lastCheck === false) {
				update_option('envato-toolkit-last-check', time());
				return true;
			}

			// check for an update every 30 min
		...
		public function check_update() {
			// check for updates
			$upgrader = $this->_get_upgrader();
			$result_update = $upgrader->check_for_theme_update();

			if (isset($result_update->errors) & !empty($result_update->errors)) {
				update_option('envato-toolkit-last-check', false);
				update_option('envato-toolkit-last-errors', $result_update->errors);

				add_action('admin_notices', 'envato_toolkit_admin_error_notices');
				return false;
			}

			// update the time we last checked
			update_option('envato-toolkit-last-check', time());
			update_option('envato-toolkit-last-errors', false);

			// add update alert, to update the theme
			if (isset($result_update->updated_themes_count) && $result_update->updated_themes_count > 0 ) {
				add_action('admin_notices', 'envato_toolkit_admin_notices');
				update_option('envato-toolkit-has-update', $result_update->updated_themes_count);
				return true;
			} else {
				update_option('envato-toolkit-has-update', 0);
				return false;
			}
		}
		
		public function do_updgrade() {
			if ($this->check_update()) {
				$upgrader = $this->_get_upgrader();
				
				$result_upgrade = $upgrader->upgrade_theme();
				
				if (!$result_upgrade->success) {
					if (isset($result_upgrade->errors) && !empty($result_upgrade->errors)) {
						update_option('envato-toolkit-last-upgrade-errors', array_shift($result_upgrade->errors));
					} elseif (isset($result_upgrade->installation_feedback) && !empty($result_upgrade->installation_feedback)) {
						update_option('envato-toolkit-last-upgrade-errors', array_shift($result_upgrade->installation_feedback));
					} else {
						update_option('envato-toolkit-last-upgrade-errors', __('Undefined update error', 'dfd'));
					}
					
					add_action('admin_notices', 'envato_toolkit_admin_upgrade_error_notices');
					return false;
				} else {
					update_option('envato-toolkit-last-upgrade-errors', false);
					add_action('admin_notices', 'envato_toolkit_admin_upgrade_notices');
					return true;
				}
				
			} else {
				update_option('envato-toolkit-last-upgrade-errors', __('Has no updates', 'dfd'));
				add_action('admin_notices', 'envato_toolkit_admin_upgrade_error_notices');
				return false;
			}
		}
	...
	function envato_toolkit_credentials_admin_notices() {
		...
		$message = sprintf(
				__("To enable theme update notifications, please enter your Envato Marketplace credentials in the %s", 'dfd'),
				"<a href='" . admin_url() . "admin.php?page=redux_options&tab=10'>'{$theme_name}' Update Options</a>"
			);
	...
	function envato_toolkit_admin_notices() {
		...
		$message = sprintf(
				__("An update to the theme is available! Head over to %s to update it now.", 'dfd'),
				"<a href='" . admin_url() . "admin.php?page=redux_options&tab=10'>'{$theme_name}' Update Options</a>"
			);
		echo "<div id='message' class='updated below-h2'><p>{$message}</p></div>";
	}

	function envato_toolkit_admin_error_notices() {
		$lastErrors = get_option( 'envato-toolkit-last-errors' );

		if (empty($lastErrors) || !isset($lastErrors[1])) {
			return;
		}

		echo '<div class="error">'.$lastErrors[1].'</div>';

		update_option('envato-toolkit-last-errors', false);
	}

	function envato_toolkit_admin_upgrade_notices() {
		$theme_name = function_exists( 'wp_get_theme' ) ? wp_get_theme()->Name : get_current_theme();

		$message = sprintf(__("Theme '%s' was updated", "dfd"), $theme_name);
		echo "<div id='message' class='updated below-h2'><p>{$message}</p></div>";
	}
	
	function envato_toolkit_admin_upgrade_error_notices() {
		$errors = get_option('envato-toolkit-last-upgrade-errors');
		
		if (is_array($errors) && isset($errors[1])) {
			$errors =  $errors[1];
		}
