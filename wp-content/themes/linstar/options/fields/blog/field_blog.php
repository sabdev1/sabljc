...
?>
	...
	<table class="form-table" id="blog-table-opt" style="border: none">
		<tr>
			<th scope="row"><?php _e( 'Front page displays', 'king' ); ?></th>
			<td id="front-static-pages">
				<fieldset>
					<legend class="screen-reader-text"><span><?php _e( 'Front page displays', 'king' ); ?></span></legend>
					<p><label>
					...
					<p><label>
						...
						<?php printf( __( 'A <a href="%s">static page</a> (select below)', 'king' ), 'edit.php?post_type=page' ); ?>
					</label>
					</p>
					<ul>
						<li><label for="page_on_front"><?php printf( __( 'Front page: %s' , 'king'), wp_dropdown_pages( array( 'name' => $this->args['opt_name'].'[page_on_front]', 'echo' => 0, 'show_option_none' => __( '&mdash; Select &mdash;', 'king' ), 'option_none_value' => '0', 'selected' => get_option( 'page_on_front' ) ) ) ); ?></label></li>
						<li><label for="page_for_posts"><?php printf( __( 'Posts page: %s' , 'king'), wp_dropdown_pages( array( 'name' => $this->args['opt_name'].'[page_for_posts]', 'echo' => 0, 'show_option_none' => __( '&mdash; Select &mdash;' , 'king'), 'option_none_value' => '0', 'selected' => get_option( 'page_for_posts' ) ) ) ); ?></label></li>
					</ul>
				<?php if ( 'page' == get_option( 'show_on_front' ) && get_option( 'page_for_posts' ) == get_option( 'page_on_front' ) ) : ?>
				<div id="front-page-warning" class="error inline"><p><?php _e( '<strong>Warning:</strong> these pages should not be the same!' , 'king'); ?></p>
				</div>
				<?php endif; ?>
			</fieldset></td>
			</tr>
			<?php endif; ?>
			<tr>
			<th scope="row"><label for="blog-layout"><?php _e( 'Blog Layout' , 'king'); ?></label></th>
			<td>
				<?php
					global $king;
					$std = 'large';
			...
			<tr>			
			<tr>
				<th scope="row"><label for="blog-layout"><?php _e( 'Categories for blog' , 'king'); ?></label></th>
				<td>
					<?php
						global $king;
						$std = array();
			...
			<tr>
			<th scope="row"><label for="posts_per_page"><?php _e( 'Blog pages show at most' , 'king'); ?></label></th>
			<td>
			<input name="<?php echo esc_attr( $this->args['opt_name'] ); ?>[posts_per_page]" type="number" step="1" min="1" id="posts_per_page" value="<?php form_option( 'posts_per_page' ); ?>" class="small-text regular-text" /> <?php _e( 'posts' , 'king'); ?>
			</td>
			</tr>
			<tr>
			<th scope="row"><label for="posts_per_rss"><?php _e( 'Syndication feeds show the most recent', 'king' ); ?></label></th>
			<td><input name="<?php echo esc_attr( $this->args['opt_name'] ); ?>[posts_per_rss]" type="number" step="1" min="1" id="posts_per_rss" value="<?php form_option( 'posts_per_rss' ); ?>" class="small-text regular-text" /> <?php _e( 'items', 'king' ); ?></td>
			</tr>
			<tr>
			<th scope="row"><?php _e( 'For each article in a feed, show' , 'king'); ?> </th>
			<td><fieldset><legend class="screen-reader-text regular-text"><span><?php _e( 'For each article in a feed, show', 'king' ); ?> </span></legend>
			<p><label><input name="<?php echo esc_attr( $this->args['opt_name'] ); ?>[rss_use_excerpt]" type="radio" value="0" <?php checked( 0, get_option( 'rss_use_excerpt' ) ); ?>	/> <?php _e( 'Full text', 'king' ); ?></label><br />
			<label><input name="<?php echo esc_attr( $this->args['opt_name'] ); ?>[rss_use_excerpt]" type="radio" value="1" <?php checked( 1, get_option( 'rss_use_excerpt' ) ); ?> /> <?php _e( 'Summary', 'king' ); ?></label></p>
			</fieldset></td>
		</tr>
	</table>
