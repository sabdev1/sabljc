...
class King_User{
	...
	public function init(){
    	...
    	wp_localize_script( 'king-user-script', 'ajax_user_object', array( 
	        ...
	        'loadingmessage' => __('Sending user info, please wait...', KING_DOMAIN)
	    ));
			...
			'linstar_addition_fields' => array(
				...
				'fields' => array(
					...
					'country' => array(
						...
						'type'        => 'select',
						'options'     => array( '' => __( 'Select a country&hellip;', KING_DOMAIN ) ) + $this->get_countries()
					)
				)				
			)			
		));
	...
	public function login(){
	    ...
	    if ( is_wp_error($user_signon) ){
			$output = array(
				'loggedin' => false, 
				'message' => __('Wrong username or password.', KING_DOMAIN)
			);			
	    } else {
			$output = array(
				'loggedin' => true, 
				'message' => __('Login successful, redirecting...', KING_DOMAIN)
			);			     
	    }
		wp_send_json($output);
	...
	public function register(){
		...
		if ( ! get_option( 'users_can_register' ) ) {
			$output = array(
				'status' => false, 
				'message' => __('Disabled user register.', KING_DOMAIN)
			);
			wp_send_json($output);	
			die();
		}
		...
		if(!$this->checkEmail($email)){
			$output = array(
				'loggedin' => false, 
				'message' => __('Enter a correct email.', KING_DOMAIN)
			);
			wp_send_json($output);	
			die();
		}
		...
		if($password1 != $password2){
			$output = array(
				'loggedin' => false, 
				'message'=>__('Password does not match.', KING_DOMAIN)
			);
			wp_send_json($output);				
			die();
		}
		...
		if(strlen($password1) < 6){
			$output = array(
				'loggedin' => false, 
				'message'=>__('Password too short.', KING_DOMAIN)
			);
			wp_send_json($output);
			die();
		}
		...
		if ( is_wp_error($user_register) ){
			...
			if(in_array('empty_user_login', $error))
				wp_send_json(
					array(
						'loggedin' => false, 
						'message' => __('Empty user login', KING_DOMAIN)
					)
				);
			elseif(in_array('existing_user_login',$error))
				wp_send_json(
					array(
						'loggedin' => false, 
						'message' => __('This username is already registered.', KING_DOMAIN)
					)
				);
			elseif(in_array('existing_user_email',$error))
		        wp_send_json(
					array(
						'loggedin' => false, 
						'message' => __('This email address is already registered.', KING_DOMAIN)
					)
				);
		} else {
	...
	public function forgot_password(){
		...
		if($this->checkEmail($email)){
			if(email_exists($email)){
				...
				if($request_pw){
					...
					wp_send_json(
						array(
							'status' => true, 
							'message' => __( "Please check your email and reset your password.", KING_DOMAIN )
						)
					);	
				}else{
				...
				wp_send_json(
					array(
						'status'=>false, 
						'message'=>__('This email not exist in my system.', KING_DOMAIN)
					)
				);
			}
		}else{
			wp_send_json(
				array(
					'status'=>false, 
					'message'=>__('Enter a validate email.', KING_DOMAIN)
				)
			);
		}
