...
class Su_Tools {
	...
	public static function example() {
		...
		// Cache not found
			...
			// Check for code
			if ( !$code ) die( '<p class="king-examples-error">' . __( 'Example code does not found, please check it later', KING_DOMAIN ) . '</p>' );
			// Clean-up the code
			...
			foreach ( $chunks as $chunk ) {
				...
				echo '<div class="king-examples-code"><span class="king-examples-get-code button"><i class="fa fa-code"></i>&nbsp;&nbsp;' . __( 'Get the code', KING_DOMAIN ) . '</span><textarea rows="' . $rows . '">' . esc_textarea( $chunk ) . '</textarea></div>';
			}
	...
	public static function access() {
		if ( !self::access_check() ) wp_die( __( 'Access denied', KING_DOMAIN ) );
	}
	...
	public static function slide_link_input( $form_fields, $post ) {
		$form_fields['su_slide_link'] = array(
			'label' => __( 'Slide link', KING_DOMAIN ),
			'input' => 'text',
			'value' => get_post_meta( $post->ID, 'su_slide_link', true ),
			'helps' => sprintf( '<strong>%s</strong><br>%s', __( 'king-theme.com', KING_DOMAIN ), __( 'Use this field to add custom links to slides used with Slider, Carousel and Custom Gallery components', KING_DOMAIN ) )
		);
