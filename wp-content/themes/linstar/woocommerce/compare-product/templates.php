...
<h1>
	...
	?>
    <?php if ( ! $is_iframe ) : ?><a class="close" href="#"><?php _e( 'Close window [X]', 'king' ) ?></a><?php endif; ?>
</h1>
...
<table class="compare-list" width="100%" cellpadding="0" cellspacing="0"<?php if ( empty( $products ) ) echo ' style="width:100%"' ?>>
    ...
    <?php if ( empty( $products ) ) : ?>
        ...
        <tr class="no-products">
            <td><?php _e( 'No products added in the compare table.', 'king' ) ?></td>
        </tr>
        ...
        <tr class="remove">
            ...
            <?php foreach( $products as $i => $_product ) : $product_class = ( $i % 2 == 0 ? 'odd' : 'even' ) . ' product_' . $_product->id ?>
                <td class="<?php echo esc_attr( $product_class ); ?>">
                    <a href="<?php echo add_query_arg( 'redirect', 'view', $this->remove_product_url( $_product->id ) ) ?>" data-product_id="<?php echo esc_attr( $_product->id ); ?>"><?php _e( 'Remove', 'king' ) ?> <span class="remove">x</span></a>
                </td>
            <?php endforeach ?>
        </tr>
