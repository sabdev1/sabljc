...
if( !class_exists( 'king_WISHLIST_UI' ) ) {
    class king_WISHLIST_UI {
        ...
        public static function add_to_wishlist_button( $url, $product_type, $exists ) { 
            ...
            $html .= '<div class="king-wishlist-wishlistaddedbrowse disappear" style="display:none;"><span class="feedback">' . __( 'Product added!','king' ) . '</span> <a href="' . esc_url( $url ) . '">' . apply_filters( 'king-wishlist-browse-wishlist-label', __( 'Browse Wishlist', 'king' ) ) . '</a></div>';
            $html .= '<div class="king-wishlist-wishlistexistsbrowse ' . ( $exists ? 'displaying' : 'disappear' ) . '" style="display:' . ( $exists ? 'block' : 'none' ) . '"><span class="feedback">' . __( 'This product was added to wishlist!', 'king' ) . '</span> <a href="' . esc_url( $url ) . '">' . apply_filters( 'king-wishlist-browse-wishlist-label', __( 'Go to Wishlist', 'king' ) ) . '</a></div>';
        ...
        public static function add_to_cart_button( $product_id, $stock_status ) {
    		...
    		$label = $product->product_type == 'variable' ? apply_filters( 'variable_add_to_cart_text', __('Select options', 'king') ) : apply_filters( 'king_wishlist_add_to_cart_label', $king->cfg['wl_label'] );
        ...
        public static function get_share_links( $url ) {
            ...
            $html .= apply_filters( 'king_wishlist_socials_share_title', '<h4>' . __( 'Share on:', 'king' ) . '</h4>' );
                $html .= '<ul>';
                
                if( $king->cfg['wl_facebook'] == 1 )
                    { $html .= '<li class="globalBgColor"><a target="_blank" href="https://www.facebook.com/sharer.php?s=100&amp;p[title]=' . $title . '&amp;p[url]=' . $url . '&amp;p[summary]=' . $summary . '&amp;p[images][0]=' . $imageurl . '" title="' . __( 'Facebook', 'king' ) . '">&nbsp;<i class="fa fa-facebook fa-lg"></i>&nbsp;</a></li>'; }
                
                if( $king->cfg['wl_twitter'] == 1 )
                    { $html .= '<li class="globalBgColor"><a target="_blank" href="https://twitter.com/share?url=' . $url . '&amp;text=' . $twitter_summary . '" title="' . __( 'Twitter', 'king' ) . '"><i class="fa fa-twitter fa-lg"></i></a></li>'; }
